...
class ResultNL:
    ...
    @staticmethod
    def convert_to_result(lam_nl, res, mesh, geometry, thickness):
        freq = np.sqrt(lam_nl)
        
        u10 = res[range(0,6 * mesh.nodes_count(),6)]
        u11 = res[range(1,6 * mesh.nodes_count(),6)]
        u12 = res[range(2,6 * mesh.nodes_count(),6)]
        
        u30 = res[range(3,6 * mesh.nodes_count(),6)]
        u31 = res[range(4,6 * mesh.nodes_count(),6)]
        u32 = res[range(5,6 * mesh.nodes_count(),6)]
        
        return ResultNL(freq, u10, u11, u12, u30, u31, u32, mesh, geometry, thickness)
