...
class ResultNL:
    ...
    @staticmethod
    def convert_to_result(lam_nl, res, mesh, geometry):
    
        freq = np.sqrt(lam_nl)
        u = res[range(0,3 * mesh.nodes_count(),3)]
        g = res[range(1,3 * mesh.nodes_count(),3)]
        w = res[range(2,3 * mesh.nodes_count(),3)]
        return ResultNL(freq, u, g, w, mesh, geometry)
